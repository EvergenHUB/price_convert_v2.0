import pandas as pd
temp_df = pd.DataFrame() # Задаем пустой dataframe чтобы потом в него записывать данные полученые из файлов
...
for file_name in ['1.xlsx','2.xlsx','3.xlsx','4.xlsx',
    ...
    sobr_tab = (pd.DataFrame(list(okruglenie.items()), columns=['Name', 'Price']))  # формируем таблицу с двумя столбцами
    ...
    temp_df = temp_df.append(sobr_tab, ignore_index=True) # Вприсываем данные из файлов в одну таблицу

# Меняем имена товаров на id товаров в созданом столбце
zamena_na_id = temp_df.replace({'id': {"Аквамарин": 9205, "Алтай": 9241, "Альба": 9223, "Альба СТ": 9317,
...
zamena_na_id.to_excel("all_id_price.xlsx", sheet_name='Цены и id', index=False)
...
for path_file in ['1.xlsx', '2.xlsx', '3.xlsx', '4.xlsx', '5.xlsx',
                  '6.xlsx', '7.xlsx', '8.xlsx', '9.xlsx',
    ...
    temp_df_2 = temp_df_2.append(out_df, ignore_index=True)  # Вписываем данные из файлов в одну таблицу
    #print(temp_df)
    # Заменяем имя на id
out_df_name=temp_df_2.replace({'name': {"Аквамарин": 9205, "Алтай": 9241, "Альба": 9223, "Альба СТ": 9317,
                                            "Аметист": 24, "Амстердам": 9269, "Анастасия": 32, "Анастасия СТ": 9302,
                                            "Андерсон": 9237, "Андерсон СТ": 9331, "Ария": 3, "Атлас": 9211, "Балу": 17,
                                            "Беатрис": 9217, "Беатрис СТ": 9311, "Белла": 35, "Белла СТ": 9305,
                                            "Берилл": 25, "Берлин": 9267, "Бирюза": 9207, "Блэкберн": 9256, "Блюз": 5,
...
out_df_razmer = out_df_name.replace({'razmer': {"60*120": 147, "65*125": 148, "70*140": 149, "70*160": 150,
                                                    ...
                                                    "190х200": 142, " 200х186": 143, "200х186": 143, " 200х190": 144,
                                                    "200х190": 144, " 200х195": 145, "200х195": 145,
...
out_df_itog.to_excel("all_razmers_id.xlsx", index=None, header=headers, sheet_name='Цены размеров с ID') # сохранение в эксель
